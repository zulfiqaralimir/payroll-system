...
export default function PayrollDetail() {
  ...
  const fetchData = async () => {
    ...
    try {
      const r = await api.get(`/payroll/period/${month}/${year}`);
  ...
  if (err) return (
    <div className="p-8 text-center">
      ...
      <button onClick={() => router.push('/payroll')} className="text-blue-600 underline text-sm">Back to Payroll</button>
    </div>
  );
  ...
  if (rows.length === 0) return (
    <div className="p-8 text-center text-gray-400">
      ...
      <button onClick={() => router.push('/payroll')} className="text-blue-600 underline text-sm mt-2 block mx-auto">Back to Payroll</button>
    </div>
  );
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        ...
        {status === 'draft' && (
          <button onClick={handleSubmit} disabled={submitting}
            ...
            {submitting
              ? <><svg className="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>Submitting...</>
              : <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>Submit for CFO Approval</>
            }
          </button>
        )}
      </div>

      {submitMsg && <div className="bg-green-50 border border-green-200 text-green-700 rounded-lg px-4 py-3 text-sm">{submitMsg}</div>}
      {submitErr && <div className="bg-red-50 border border-red-200 text-red-700 rounded-lg px-4 py-3 text-sm">{submitErr}</div>}
      ...
      <div className="flex items-center gap-3">
        ...
        {fDept && <button onClick={() => setFDept('')} className="text-xs text-gray-400 hover:text-gray-600">Clear</button>}
      </div>
