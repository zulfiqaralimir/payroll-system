    ...
    <div className="p-6 space-y-5">
      ...
      {!loading && data && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          {tab === 'summary'  && <SummaryReport  data={data} month={month} year={year} />}
...
function SummaryReport({ data, month, year }) {
          ...
          {rows.map((r, i) => (
            <tr key={i} className="hover:bg-gray-50">
              ...
              <td className="px-5 py-3">
                <span className={`px-2 py-0.5 rounded text-xs font-medium ${r.staff_type === 'admin' ? 'bg-blue-50 text-blue-700' : 'bg-green-50 text-green-700'}`}>
                  {r.staff_type}
                </span>
              </td>
...
function BankReport({ data }) {
  ...
  return (
    <div>
            ...
            <tbody className="divide-y divide-gray-50">
              ...
              <tr className="bg-gray-50 font-semibold">
                ...
                <td className="px-5 py-2 text-right font-mono text-gray-900">
                  PKR {PKR(emps.reduce((a, r) => a + Number(r.amount), 0))}
                </td>
              </tr>
            </tbody>
          </table>
      ...
      {rows.length === 0 && (
        <div className="py-12 text-center text-gray-400 text-sm">No bank transfer data for this period.</div>
      )}
    </div>
  );
}
    ...
    <div className="overflow-x-auto">
      ...
      <table className="w-full text-sm mt-4">
        ...
        <tbody className="divide-y divide-gray-50">
          ...
          {rows.length === 0 && (
            <tr><td colSpan={9} className="py-10 text-center text-gray-400 text-sm">No PF/EOBI data for this period.</td></tr>
          )}
        </tbody>
      </table>
    </div>
    ...
    <div className="overflow-x-auto">
      ...
      <table className="w-full text-sm mt-4">
        ...
        <tbody className="divide-y divide-gray-50">
          ...
          {rows.length === 0 && (
            <tr><td colSpan={8} className="py-10 text-center text-gray-400 text-sm">No tax data for this period.</td></tr>
          )}
        </tbody>
      </table>
    </div>
...
function JvReport({ data }) {
  ...
  const txns = {};
  rows.forEach(r => {
    if (!txns[r.transaction_no]) txns[r.transaction_no] = [];
    txns[r.transaction_no].push(r);
  });
  ...
  return (
    <div>
              ...
              {entries.map((r, i) => (
                <tr key={i} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-2 text-right font-mono text-gray-700">
                    {Number(r.debit_amount) > 0 ? PKR(r.debit_amount) : '—'}
                  </td>
                  <td className="px-5 py-2 text-right font-mono text-gray-700">
                    {Number(r.credit_amount) > 0 ? PKR(r.credit_amount) : '—'}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {rows.length === 0 && (
        <div className="py-12 text-center text-gray-400 text-sm">No JV entries for this period. Run payroll first.</div>
      )}
