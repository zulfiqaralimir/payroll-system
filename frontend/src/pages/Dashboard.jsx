        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <SummaryCard
              ...
              value={Number(stats.activeEmployees).toLocaleString()}
              sub={`${stats.activeDepartments} departments`}
              icon={<PeopleIcon />}
              color="blue"
            />
            <SummaryCard
              label="Latest Month Net"
              value={PKR(stats.thisMonthNet)}
              sub={stats.thisMonthLabel ? `${stats.thisMonthEmployees} employees · ${stats.thisMonthLabel}` : 'No data yet'}
              icon={<MoneyIcon />}
              color="green"
            />
            <SummaryCard
              label="Pending Approvals"
              value={Number(stats.pendingApprovals) > 0 ? stats.pendingApprovals : '0'}
              sub={Number(stats.pendingApprovals) > 0 ? 'Awaiting CFO review' : 'All up to date'}
              icon={<CheckIcon />}
              color={Number(stats.pendingApprovals) > 0 ? 'yellow' : 'emerald'}
            />
            <SummaryCard
              label="Payroll Periods"
              value={stats.recentPayrolls?.length || 0}
              sub="Processed periods"
              icon={<CalIcon />}
              color="purple"
            />
          </div>
                ...
                {stats.recentPayrolls.map((p, i) => (
                  <div key={i} className="px-5 py-3 flex items-center justify-between hover:bg-gray-50 transition-colors">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_CLS[p.status] || 'bg-gray-100 text-gray-600'}`}>
                        {p.status}
                      </span>
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            ...
            <div className="flex flex-wrap gap-3">
              {(user?.role === 'admin' || user?.role === 'hr_manager') && (
                <>
                  <ActionBtn label="View Employees"  onClick={() => navigate('/employees')} color="blue" />
                  <ActionBtn label="Run Payroll"     onClick={() => navigate('/payroll')}   color="green" />
                  <ActionBtn label="Generate Payslips" onClick={() => navigate('/payslips')} color="teal" />
                  <ActionBtn label="Database Tables" onClick={() => navigate('/master')}    color="gray" />
                </>
              )}
              {(user?.role === 'admin' || user?.role === 'cfo') && (
                <>
                  <ActionBtn label="Approvals"  onClick={() => navigate('/approvals')} color="yellow" />
                  <ActionBtn label="Reports"    onClick={() => navigate('/reports')}   color="purple" />
                </>
              )}
...
function TrendChart({ data }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: 160 }}>
      ...
      {data.map((d, i) => (
        <g key={i}>
          <circle cx={xPos(i)} cy={yPos(d.total_net)} r="3" fill="#3b82f6"/>
          <text x={xPos(i)} y={H - PAD.b + 14} textAnchor="middle" fontSize="9" fill="#6b7280">
            {MONTHS[d.month - 1]}
          </text>
        </g>
      ))}
  ...
  return (
    <div className="overflow-y-auto" style={{ maxHeight: 260 }}>
      <svg viewBox={`0 0 ${W} ${totalH}`} className="w-full" style={{ height: totalH }}>
        {data.map((d, i) => {
          const y   = i * (BAR_H + GAP);
          const bW  = Math.max(2, ((Number(d.total_net) / max) * (W - PAD_L - PAD_R)));
          const clr = d.staff_type === 'admin' ? '#3b82f6' : '#10b981';
          return (
            <g key={i}>
              <text x={PAD_L - 6} y={y + BAR_H / 2 + 4} textAnchor="end" fontSize="10" fill="#374151"
                className="truncate">{d.department.replace(' & ', '/')}</text>
              <rect x={PAD_L} y={y} width={bW} height={BAR_H} rx="3" fill={clr} opacity="0.85"/>
              <text x={PAD_L + bW + 4} y={y + BAR_H / 2 + 4} fontSize="10" fill="#6b7280">
                {(Number(d.total_net) / 1e6).toFixed(1)}M
              </text>
            </g>
          );
        })}
      ...
      <div className="flex gap-4 mt-2 text-xs text-gray-500">
        <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-blue-500 inline-block"/>Admin Staff</span>
        <span className="flex items-center gap-1"><span className="w-3 h-3 rounded bg-emerald-500 inline-block"/>Direct Staff</span>
      </div>
    </div>
  );
}
...
function ActionBtn({ label, onClick, color }) {
  ...
  return (
    <button onClick={onClick}
      className={`border rounded-lg px-4 py-2 text-sm font-medium transition-colors ${cls[color] || cls.gray}`}>
      {label}
    </button>
  );
}
...
const PeopleIcon = () => (
  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
      d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.768-.231-1.48-.634-2.071M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.768.231-1.48.634-2.071m0 0A3 3 0 1110 10a3 3 0 013 3m-3 5c0-1.032.291-1.997.804-2.817"/>
  </svg>
);
const MoneyIcon = () => (
  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
