...
function Badge({ count, color = 'bg-blue-600' }) {
  return <span className={`ml-1.5 ${color} text-white text-xs rounded-full px-2 py-0.5`}>{count}</span>;
}
...
const COL_DEFS = {
  ...
  payslips: [
    ...
    { label: 'PDF',       render: r => r.pdf_path ? <a href={r.pdf_path} className="text-blue-600 underline text-xs" target="_blank" rel="noreferrer">Download</a> : '—' },
  ],
  'bank-transfers': [
    { label: 'Emp ID',    key: 'emp_code' },
    { label: 'Name',      key: 'emp_name' },
  ...
  'pf-eobi': [
    ...
    { label: 'PF Emp2 Share',render: r => PKR(r.pf_employer_share) },
...
export default function MasterTables() {
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      {/* Top bar */}
      <div className="bg-[#1a2540] text-white px-6 py-4 flex items-center justify-between shadow">
        <div>
          <h1 className="text-lg font-bold tracking-wide">WELLSERVE HR PAYROLL SYSTEM</h1>
          <p className="text-xs text-slate-400 mt-0.5">Wellserve Oilfield Services (Pvt) Ltd — Database Viewer</p>
        </div>
        <div className="text-sm">
          <span className={`inline-block w-2 h-2 rounded-full mr-2 ${dbStatus === 'connected' ? 'bg-green-400' : 'bg-red-400'}`}></span>
          <span className="text-slate-300">DB: {dbStatus}</span>
        </div>
      </div>
      ...
      <div className="max-w-screen-xl mx-auto px-4 py-5">
        ...
        {/* Phase 03 — Backend API Status Panel */}
        <div className="bg-white rounded-xl shadow border border-gray-200 mt-4 p-4">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-3">Phase 03 — Backend API Endpoints</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs font-mono">
            <div>
              <p className="text-blue-700 font-semibold mb-1">Master Routes (10)</p>
              {['/api/departments','/api/employees','/api/salary-structures','/api/overtime-rates',
                '/api/rig-bonus-rates','/api/travelling-rates','/api/tax-slabs',
                '/api/account-codes','/api/banks','/api/pf-schemes'].map(ep => (
                <div key={ep} className="flex items-center gap-1 text-gray-600">
                  <span className="text-green-500">✓</span>{ep}
                </div>
              ))}
            </div>
            <div>
              <p className="text-emerald-700 font-semibold mb-1">Derived Routes (7) + System</p>
              {['/api/attendance','/api/deductions','/api/payroll','/api/payslips',
                '/api/bank-transfers','/api/pf-eobi','/api/jv-entries',
                '/api/users','/api/audit-log'].map(ep => (
                <div key={ep} className="flex items-center gap-1 text-gray-600">
                  <span className="text-green-500">✓</span>{ep}
                </div>
              ))}
            ...
            <div>
              <p className="text-purple-700 font-semibold mb-1">New Phase 03 Routes</p>
              {['/api/approvals/submit/:m/:y','/api/approvals/approve/:m/:y',
                '/api/approvals/reject/:m/:y','/api/approvals/mark-paid/:m/:y',
                '/api/reports/monthly/:m/:y','/api/reports/jv/:m/:y',
                '/api/reports/bank-transfers/:m/:y','/api/reports/pf-eobi/:m/:y',
                '/api/reports/employees','/api/reports/dashboard'].map(ep => (
                <div key={ep} className="flex items-center gap-1 text-gray-600">
                  <span className="text-green-500">✓</span>{ep}
                </div>
              ))}
              <p className="text-violet-700 font-semibold mb-1 mt-2">Middleware & Controllers</p>
              {['middleware/auth.js (JWT)','middleware/roleCheck.js','middleware/errorHandler.js',
                'controllers/payrollController.js','controllers/excelController.js'].map(f => (
                <div key={f} className="flex items-center gap-1 text-gray-600">
        ...
        <p className="text-center text-xs text-gray-400 mt-4">
          WellServe HR Payroll System — Phase 03 Complete — Developed by Zulfiqar Ali Mir, Black Iron Quantum AI (Pvt) Ltd
        </p>
      </div>
    </div>
  );
