...
export default function EmployeeList() {
  ...
  const clearFilters = () => {
    setSearch(''); setFDept(''); setFStaffType(''); setFEmpType(''); setFStatus('active');
  };
                      ...
                      {emp.is_active && (
                        <>
                          ...
                          <button onClick={() => { setEditing(emp); setShowForm(true); }}
                            className="text-gray-600 hover:text-gray-800 text-xs font-medium hover:underline">
                            Edit
                          </button>
                          <span className="text-gray-200">|</span>
                          <button onClick={() => handleDeactivate(emp.id, emp.name)}
                            disabled={deactId === emp.id}
