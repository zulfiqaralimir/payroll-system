...
export default function UserManagement() {
  ...
  const handleSave = async () => {
    ...
    try {
      let r;
      if (editing) {
        r = await api.put(`/users/${editing}`, form);
      } else {
      ...
      if (r.success) {
        ...
        setFormErr(r.error || 'Save failed');
      }
    } catch (e) {
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {u.id !== me?.id && (
                          <button onClick={() => handleDeactivate(u)}
                            className="text-xs text-red-500 hover:underline font-medium">Deactivate</button>
                        )}
                      </div>
                    </td>
                  </tr>
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100">
                <p className="font-semibold text-gray-500 text-sm">
                  Deactivated Users
                  <span className="ml-2 text-xs font-normal text-gray-400">({inactive.length})</span>
                </p>
              </div>
              <table className="w-full text-sm">
                <tbody className="divide-y divide-gray-50">
                  {inactive.map(u => (
                    <tr key={u.id} className="opacity-60">
                      ...
                      <td className="px-5 py-3">
                        <span className="px-2 py-0.5 rounded text-xs bg-gray-100 text-gray-500">{ROLE_LABEL[u.role]}</span>
                      </td>
