...
export function useExcelWatcher() {
  ...
  useEffect(() => {
    ...
    return () => { socket?.off('excel_detected'); };
  }, []);

  // Import from the watch folder (folder-watcher flow)
  ...
  // Upload file directly from the user's computer (reads as base64, sends as JSON)
