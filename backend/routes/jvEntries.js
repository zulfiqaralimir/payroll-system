  ...
  try {
    ...
    const deptTotals = await pool.query(`
      SELECT d.id AS dept_id, d.name AS dept_name,
             ...
             SUM(pr.overtime_amount) AS ot_total,
             SUM(pr.rig_bonus_amount) AS rig_total,
             SUM(pr.gross_salary) AS gross_total,
             SUM(pr.net_salary) AS net_total,
             SUM(pr.eobi) AS eobi_total,
             SUM(pr.provident_fund) AS pf_total,
             SUM(pr.income_tax) AS tax_total
    ...
    let created = 0;
    for (const dept of deptTotals.rows) {
      const ac = await pool.query(
        `SELECT * FROM account_codes WHERE department_id=$1 AND category='salary' AND entry_type='debit' LIMIT 1`,
        [dept.dept_id]
      );
      const code = ac.rows[0];
      await pool.query(`
        INSERT INTO jv_entries (month, year, department_id, account_code_id, account_code, account_name, debit_amount, transaction_no, description)
        VALUES ($1,$2,$3,$4,$5,$6,$7,1,$8)
      `, [month, year, dept.dept_id, code?.id||null, code?.account_code||'', dept.dept_name + ' Salaries', dept.salary_total, `${dept.dept_name} — Salary Transaction`]);
      created++;
    }

    const totals = (await pool.query(`
      SELECT SUM(eobi) AS total_eobi, SUM(provident_fund) AS total_pf, SUM(net_salary) AS total_net
    ...
    res.json({ success: true, message: `JV entries generated (${created} departments)` });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});
