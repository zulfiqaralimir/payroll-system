export function calculateGross(employee, salaryStructure, overtimeRate, rigBonusRate, travellingRate, attendance) {
  ...
  const gross = basic + hra + utility + conveyance + overtimeAmt + rigBonusAmt + travelAmt
    + (parseFloat(attendance.annual_bonus) || 0) + (parseFloat(attendance.arrears) || 0)
    + (parseFloat(attendance.reimbursement) || 0) + (parseFloat(attendance.advance_salary) || 0)
    + (parseFloat(attendance.meal_allowance) || 0);
  return {
    basic_pay: basic, house_rent_allowance: hra, utility_allowance: utility,
    conveyance_allowance: conveyance, overtime_amount: overtimeAmt,
    rig_bonus_amount: rigBonusAmt, travelling_amount: travelAmt,
...
export function calculateTax(annualGross, taxSlabs) {
  const slab = taxSlabs.find(s =>
    annualGross >= parseFloat(s.min_income) &&
    (s.max_income === null || annualGross <= parseFloat(s.max_income))
  );
  if (!slab) return 0;
  return (parseFloat(slab.fixed_tax) + ((annualGross - parseFloat(slab.min_income)) * parseFloat(slab.tax_rate) / 100)) / 12;
}

export function calculateDeductions(employee, salaryStructure, attendance, gross, taxSlabs) {
  ...
  const tax         = calculateTax(gross * 12, taxSlabs);
  const taxAdj      = parseFloat(attendance.tax_adjustment) || 0;
  const pf          = employee.pf_member ? (parseFloat(salaryStructure.basic_pay) || 0) * 0.0833 : 0;
  const totalDed = eobi + tax + taxAdj + pf
    + (parseFloat(attendance.loan_deduction) || 0) + (parseFloat(attendance.pf_loan) || 0)
    + absentDed + lwpDed + (parseFloat(attendance.other_deductions) || 0);
  return {
    eobi, income_tax: tax + taxAdj, provident_fund: pf,
    absent_deduction: absentDed, lwp_deduction: lwpDed,
