...
export default function EmployeeDetail() {
  ...
  const handleSaved = (updated) => { setEmp(updated); setShowEdit(false); };
  ...
  if (err) return (
    <div className="p-8 text-center">
      ...
      <button onClick={() => router.push('/employees')} className="text-blue-600 underline text-sm">Back to Employees</button>
    </div>
  );
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-5">
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-4">
          <button onClick={() => router.push('/employees')} className="text-gray-400 hover:text-gray-600 transition">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              ...
              <Badge label={emp.is_active ? 'Active' : 'Inactive'} color={emp.is_active ? 'green' : 'red'} />
              <Badge label={emp.employment_type} color={emp.employment_type === 'permanent' ? 'blue' : emp.employment_type === 'contract' ? 'yellow' : 'gray'} />
            </div>
          </div>
        </div>
